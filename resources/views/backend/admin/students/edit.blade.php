    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-800">Edit Student: {{ $student->first_name }} {{ $student->last_name }}</h2>
            </div>

            <form action="{{ route('students.update', $student->id) }}" method="POST" class="px-6 py-4" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('student_code') border-red-500 @enderror"
                            id="student_code" name="student_code" value="{{ old('student_code', $student->student_code) }}" required maxlength="20">
                        @error('student_code')
                    ...
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('class_section_id') border-red-500 @enderror"
                            ...
                            @foreach($classSections as $classSection)
                                <option value="{{ $classSection->id }}" {{ old('class_section_id', $student->class_section_id) == $classSection->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('first_name') border-red-500 @enderror"
                            id="first_name" name="first_name" value="{{ old('first_name', $student->first_name) }}" required maxlength="50">
                        @error('first_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('last_name') border-red-500 @enderror"
                            id="last_name" name="last_name" value="{{ old('last_name', $student->last_name) }}" required maxlength="50">
                        @error('last_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('guardian_name') border-red-500 @enderror"
                            id="guardian_name" name="guardian_name" value="{{ old('guardian_name', $student->guardian_name) }}" required maxlength="100">
                        @error('guardian_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('guardian_phone') border-red-500 @enderror"
                            id="guardian_phone" name="guardian_phone" value="{{ old('guardian_phone', $student->guardian_phone) }}" required maxlength="15">
                    ...
                    <div>
                        ...
                        @if($student->photo)
                            <div class="mt-2">
                                ...
                                <img src="{{ Storage::url($student->photo) }}" alt="Current photo" class="h-20 w-20 object-cover rounded mt-1">
                            </div>
                        @endif
                    </div>
                </div>
