    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-800">Create New Enrollment</h2>
            </div>

            <form action="{{ route('enrollments.store') }}" method="POST" class="px-6 py-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" min="1" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('roll_number') border-red-500 @enderror"
                            id="roll_number" name="roll_number" value="{{ old('roll_number') }}" placeholder="Optional">
                        @error('roll_number')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror"
                        ...
                        <option value="Promoted" {{ old('status') == 'Promoted' ? 'selected' : '' }}>Promoted</option>
                ...
                <div class="flex items-center justify-between mt-6">
                    ...
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                        <i class="fas fa-save mr-1"></i> Create Enrollment
                    </button>
                </div>
            </form>
        </div>
