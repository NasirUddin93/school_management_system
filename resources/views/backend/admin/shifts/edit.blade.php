    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-800">Edit Shift: {{ $shift->name }}</h2>
            </div>

            <form action="{{ route('admin.shifts.update', $shift->id) }}" method="POST" class="px-6 py-4">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror"
                        id="name" name="name" value="{{ old('name', $shift->name) }}" required maxlength="50">
                    @error('name')
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="time" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('start_time') border-red-500 @enderror"
                            id="start_time" name="start_time" value="{{ old('start_time', $shift->start_time) }}">
                        @error('start_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('end_time') border-red-500 @enderror"
                            id="end_time" name="end_time" value="{{ old('end_time', $shift->end_time) }}">
                        @error('end_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex items-center justify-between mt-6">
                    <a href="{{ route('admin.shifts.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-md">
