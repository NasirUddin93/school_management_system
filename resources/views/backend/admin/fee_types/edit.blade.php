    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-800">Edit Fee Type: {{ $feeType->name }}</h2>
            </div>

            <form action="{{ route('fee_types.update', $feeType->id) }}" method="POST" class="px-6 py-4">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror"
                        id="name" name="name" value="{{ old('name', $feeType->name) }}" required maxlength="100" placeholder="e.g., Tuition Fee, Exam Fee">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror"
                        id="description" name="description" rows="3" placeholder="Description of the fee type">{{ old('description', $feeType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('recurrence') border-red-500 @enderror"
                            ...
                            <option value="one_time" {{ old('recurrence', $feeType->recurrence) == 'one_time' ? 'selected' : '' }}>One Time</option>
                            <option value="monthly" {{ old('recurrence', $feeType->recurrence) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                            <option value="yearly" {{ old('recurrence', $feeType->recurrence) == 'yearly' ? 'selected' : '' }}>Yearly</option>
                            <option value="occasionally" {{ old('recurrence', $feeType->recurrence) == 'occasionally' ? 'selected' : '' }}>Occasionally</option>
                        </select>
                        @error('recurrence')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('default_amount') border-red-500 @enderror"
                            id="default_amount" name="default_amount" value="{{ old('default_amount', $feeType->default_amount) }}" required placeholder="0.00">
                ...
                <div class="mb-4">
                    ...
                    <select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror"
                        ...
                        <option value="active" {{ old('status', $feeType->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $feeType->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-between mt-6">
                    ...
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                        <i class="fas fa-save mr-1"></i> Update Fee Type
                    </button>
                </div>
            </form>
        </div>
