    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-800">Record Single Attendance</h2>
            </div>

            <form action="{{ route('attendances.store') }}" method="POST" class="px-6 py-4">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('remarks') border-red-500 @enderror"
                        id="remarks" name="remarks" rows="3" placeholder="Optional remarks">{{ old('remarks') }}</textarea>
                    @error('remarks')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between mt-6">
                    ...
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                        <i class="fas fa-save mr-1"></i> Record Attendance
                    </button>
                </div>
            </form>
        </div>
